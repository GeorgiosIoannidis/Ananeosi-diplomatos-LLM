  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 3) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd("Πρέπει να είστε πολήτης Ε.Ε.")
          : skipToEnd("Your must be an E.U. citizent.");
      } else if (currentQuestion === 1 && selectedRadioButtonIndex === 3) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd(
              "Πρέπει να διαμένεται ή να σπουδάζεται στην Ελλάδα για έξι μήνες."
            )
          : skipToEnd(
              "You must live or study in Greece for at least 6 months."
            );
      } else if (currentQuestion === 2 && selectedRadioButtonIndex === 1) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd("Η άδεια οδήγησής σας πρέπει να έχει λήξει ή αφαιρεθεί.")
          : skipToEnd("Your license must have expired or removed.");
        ...
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd("Πρέπει οι γιατροί να σας χαρακτηρίσουν «ΙΚΑΝΟ».")
          : skipToEnd('Doctors must have judged you "Eligible".');
      } else if (currentQuestion === 4 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd("Πρέπει να έχετε όλα τα απαραίτητα αποτελέσματα.")
          : skipToEnd("You must have all the needed results.");
      } else {
